...
pub fn handle_console_command(ui: &Ui, state: &mut UiState, dbg: &Arc<RwLock<Debugger>>) {
    let tokens: Vec<&str> = state.console_str.to_str().split_whitespace().collect();

    //match tokens[0] {
    //    "step" => println!("stepping"),
    //    _ => {} 
    //}
...
pub fn build_console_window(ui: &Ui, dbg: &Arc<RwLock<Debugger>>, 
    ...
    ChildWindow::new("log").size([0.0, 0.0]).border(true).build(ui, || {
    ...
    if ui.input_text(im_str!(""), &mut state.console_str)
        ...
        handle_console_command(ui, state, &dbg);
    }
}
...
pub fn ui_thread_loop(ui: &mut Ui, state: &mut UiState, 
    ...
    let console = Window::new(im_str!("Console Output"))
        ...
        .size([1195.0, 465.0], Condition::Always);
...
pub fn emu_thread_loop(dbg: Arc<RwLock<Debugger>>, bus: Arc<RwLock<Bus>>) {
    let mut cpu = Cpu::new(dbg.clone(), bus.clone());

    // For now, we run the bus and CPU in lock-step.
    for _i in 0..1000 {
...
fn main() {
    ...
    thread::spawn(move || {
        emu_thread_loop(emu_debugger, emu_bus);
    });
    ...
    ui_system.main_loop(move |run, ui| {
        ui_thread_loop(ui, &mut ui_state, debugger.clone(), bus.clone(), run);
    });
}
